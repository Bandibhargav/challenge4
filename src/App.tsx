...
export default function App() {
  ...
  const pendingChecklistControllerRef = useRef<AbortController | null>(null);
  const pendingChatControllerRef = useRef<AbortController | null>(null);
  ...
  useEffect(() => {
    return () => {
      pendingChecklistControllerRef.current?.abort();
      pendingChatControllerRef.current?.abort();
    };
  }, []);
  ...
  const handleGenerateAiChecklist = async (id: string) => {
    ...
    pendingChecklistControllerRef.current = controller;

    setIsGeneratingChecklistId(id);
    const incident = incidents.find((inc) => inc.id === id);
    if (!incident) {
      pendingChecklistControllerRef.current = null;
    ...
    try {
      ...
      if (pendingChecklistControllerRef.current === controller) {
        pendingChecklistControllerRef.current = null;
      }
      setIsGeneratingChecklistId(null);
    }
  ...
  const handleSendMessage = async (text: string) => {
    pendingChatControllerRef.current?.abort();
    const controller = new AbortController();
    pendingChatControllerRef.current = controller;
    ...
    try {
      ...
      if (pendingChatControllerRef.current === controller) {
        pendingChatControllerRef.current = null;
      }
  ...
  return (
    ...
    >
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 space-y-6" id="main-dashboard-grid">
        <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5 shadow-2xl shadow-slate-950/40" aria-labelledby="operations-playbook-heading">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-2xl space-y-3">
              ...
              <h2 id="operations-playbook-heading" className="text-xl font-semibold text-white">
                Operational playbook
              </h2>
              <p className="text-sm leading-6 text-slate-300">
                This command center supports live crowd intelligence, transit coordination, multilingual assistance, and accessibility-first operations for high-pressure stadium events.
              </p>
            </div>
            <div className="rounded-xl border border-slate-800 bg-slate-950/80 p-4 text-sm text-slate-300">
              <p className="font-semibold text-white">Recommended workflow</p>
              <ul className="mt-2 space-y-2 text-xs text-slate-400">
                <li className="flex items-center gap-2"><ChevronRight className="w-3.5 h-3.5 text-emerald-400" />Monitor occupancy and incidents in real time</li>
                <li className="flex items-center gap-2"><ChevronRight className="w-3.5 h-3.5 text-emerald-400" />Request AI checklists for urgent response</li>
                <li className="flex items-center gap-2"><ChevronRight className="w-3.5 h-3.5 text-emerald-400" />Use the accessibility hub for inclusive operations</li>
              </ul>
            </div>
          </div>
        </section>
